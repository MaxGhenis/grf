...
std::vector<double> LLCausalPredictionStrategy::predict(
  ...
  for (const auto& i: indices) {
    ...
    for (size_t j = 0; j < num_variables; ++j){
      ...
      X(i,j+1) = test_data->get(sampleID, current_predictor) - original_data->get(index, current_predictor);

    }

    // Treatment (just copied)
    X(i, treatment_index) = original_data->get(index, num_total_predictors+1);
